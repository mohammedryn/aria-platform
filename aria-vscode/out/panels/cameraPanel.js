...
exports.CameraPanel = void 0;
const vscode = require("vscode");
const logger_1 = require("../utils/logger");
const ariaPanel_1 = require("./ariaPanel");
...
class CameraPanel {
    constructor(panel, extensionUri) {
        ...
        this._panel.webview.onDidReceiveMessage(async (message) => {
            switch (message.command) {
                case 'imageCaptured':
                    logger_1.Logger.log("[CameraPanel] Image captured.");
                    ...
                    if (ariaPanel_1.AriaPanel.currentPanel) {
                        await ariaPanel_1.AriaPanel.currentPanel.analyzeImage(message.base64);
                    }
                    else {
                        logger_1.Logger.log("[CameraPanel] Failed to open AriaPanel for analysis.");
                    }
                    this.dispose();
                    return;
                case 'cameraError':
                    logger_1.Logger.log(`[CameraPanel] Camera access denied: ${message.text}. Switching to Bridge.`);
                ...
                case 'triggerCapture':
                    ...
                    if (message.url) {
                        try {
                            ...
                            logger_1.Logger.log("[CameraPanel] Trigger failed: " + err);
                        }
                    }
                    return;
                case 'log':
                    logger_1.Logger.log(`[CameraPanel] ${message.text}`);
                    return;
            }
        }, null, this._disposables);
    }
    async _handleCameraFallback() {
        try {
            // Use CameraBridge in STREAM mode
            // This will open external browser (to get camera) but stream video HERE
            const base64 = await cameraBridge_1.CameraBridge.capture('stream', (url) => {
                logger_1.Logger.log(`[CameraPanel] Streaming Bridge ready at ${url}. Embedding stream in panel.`);
            ...
            logger_1.Logger.log(`[CameraPanel] Bridge fallback failed: ${err}`);
        }
    }
    static createOrShow(extensionUri) {
        ...
        const panel = vscode.window.createWebviewPanel('ariaCamera', 'A.R.I.A. Camera', column, {
            enableScripts: true,
...
exports.CameraPanel = CameraPanel;
