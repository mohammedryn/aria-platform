...
class GeminiClient {
    ...
    static async uploadFile(filePath, mimeType) {
        const config = vscode.workspace.getConfiguration('aria');
        ...
        logger_1.Logger.log(`[A.R.I.A] Starting upload for ${name} (${size} bytes)...`);
        ...
        logger_1.Logger.log(`[A.R.I.A] Upload complete. URI: ${result.file.uri}`);
        return result.file.uri;
    }
    static async waitForFileActive(fileUri) {
        const config = vscode.workspace.getConfiguration('aria');
        const apiKey = config.get('apiKey') || process.env.GEMINI_API_KEY;
        if (!apiKey)
            throw new Error("API Key missing");
        logger_1.Logger.log(`[A.R.I.A] Waiting for file processing: ${fileUri}...`);
        // Polling loop
        for (let i = 0; i < 60; i++) { // Max 120 seconds
            ...
            const data = await response.json();
            logger_1.Logger.log(`[A.R.I.A] File state: ${data.state}`);
    ...
    static async analyzeVideo(fileUri, mimeType, userPrompt) {
        ...
        try {
            logger_1.Logger.log(`[A.R.I.A] Analyzing Video with Model: ${preferredModel}...`);
            const response = await this.callGemini(apiKey.trim(), preferredModel, payload);
            return this.parseResponse(response);
        }
        catch (error) {
            logger_1.Logger.log(`[A.R.I.A] Video analysis failed: ${error}`);
            throw error;
        }
    }
    ...
    static async chatWithImage(base64Image, userPrompt) {
        const config = vscode.workspace.getConfiguration('aria');
        let apiKey = config.get('apiKey') || process.env.GEMINI_API_KEY;
        ...
        try {
            ...
            logger_1.Logger.log(`[A.R.I.A] Chatting with Image using Model: gemini-3-pro-image-preview...`);
            const response = await this.callGemini(apiKey, preferredModel, payload);
            return this.parseResponse(response);
        }
        catch (error) {
            logger_1.Logger.log(`[A.R.I.A] Chat with Image failed: ${error}`);
            // Fallback to flash if pro fails
            try {
                const fallback = "gemini-3-flash-preview";
                logger_1.Logger.log(`[A.R.I.A] Retrying with ${fallback}...`);
    ...
    static async chat(userPrompt, context) {
        const config = vscode.workspace.getConfiguration('aria');
        let apiKey = config.get('apiKey') || process.env.GEMINI_API_KEY;
        const preferredModel = config.get('apiModel') || "gemini-3-flash-preview";
        if (!apiKey) {
        ...
        for (const model of modelsToTry) {
            try {
                logger_1.Logger.log(`[A.R.I.A] Chatting with Model: ${model}...`);
                const response = await this.callGemini(apiKey, model, payload);
                return this.parseResponse(response);
            }
            catch (error) {
                lastError = error instanceof Error ? error : new Error(String(error));
                logger_1.Logger.log(`[A.R.I.A] Chat failed with ${model}: ${lastError.message}`);
            }
        }
        throw lastError || new Error("All models failed");
    }
