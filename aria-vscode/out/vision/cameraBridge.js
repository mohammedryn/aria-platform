    ...
    static async capture(mode = 'external', onServerReady) {
        ...
        if (mode === 'native-python') {
            return new Promise((resolve, reject) => {
                ...
                logger_1.Logger.log(`[CameraBridge] Launching Python Bridge: ${scriptPath}`);
                ...
                this._pythonProcess.stdout?.on('data', (data) => {
                    ...
                    for (const line of lines) {
                        ...
                        if (trimmed.includes('Server started on port')) {
                            ...
                            logger_1.Logger.log(`[CameraBridge-Py] Bridge Ready at ${serverUrl}`);
                            if (onServerReady)
                ...
                this._pythonProcess.stderr?.on('data', (data) => {
                    logger_1.Logger.log(`[CameraBridge-Py Error] ${data.toString()}`);
                });
        ...
        return new Promise((resolve, reject) => {
            ...
            this._server.listen(0, '127.0.0.1', () => {
                ...
                if (address && typeof address !== 'string') {
                    ...
                    logger_1.Logger.log(`[CameraBridge] Server running at ${url}`);
                    if (onServerReady)
                        onServerReady(url);
...
<body>
    ...
    static _getHtml(mode) { return ""; } // Unused in new logic
    static async recordVideo(onServerReady) {
        this._latestFrame = null;
        this._streamClients = [];
        this._killPython();
        return new Promise((resolve, reject) => {
            const scriptPath = path.join(__dirname, '../../src/scripts/camera_server.py');
            logger_1.Logger.log(`[CameraBridge] Launching Python Bridge (Video): ${scriptPath}`);
            this._pythonProcess = cp.spawn('python', [scriptPath]);
            let serverUrl = '';
            this._pythonProcess.stdout?.on('data', (data) => {
                const output = data.toString();
                ...
                for (const line of lines) {
                    ...
                    if (trimmed.includes('VIDEO_FILE:')) {
                        ...
                        logger_1.Logger.log(`[CameraBridge-Py] Video file reported: ${filePath}`);
                        this._killPython();
                        resolve(filePath);
                    }
                }
    ...
    static triggerStartRecord(url) {
        this._postRequest(url, '/start_record');
    }
    static triggerStopRecord(url) {
        this._postRequest(url, '/stop_record');
    }
    static _postRequest(baseUrl, path) {
        try {
            ...
            req.on('error', (e) => {
                logger_1.Logger.log(`[CameraBridge] Error triggering ${path}: ${e}`);
            });
            req.end();
        }
        catch (e) {
            logger_1.Logger.log(`[CameraBridge] Invalid URL for POST: ${e}`);
        }
    }
}
exports.CameraBridge = CameraBridge;
CameraBridge._server = null;
CameraBridge._pythonProcess = null;
CameraBridge._port = 0;
CameraBridge._latestFrame = null;
CameraBridge._streamClients = [];
