...
import { VisionResult } from '../vision/visionClient';

export interface HardwareValidationResult {
    status: "pass" | "warning" | "fail";
    issues: string[];
    recommendations: string[];
    pinUsage: Record<string, string>; // Pin -> Usage Description (Changed from Map for JSON safety)
    peripherals: string[]; // Detected peripherals for simulation
}
...
export class HardwareValidator {
     ...
     */
    public static validate(code: string, hardwareSummary: string, visionContext?: VisionResult | null): HardwareValidationResult {
        const issues: string[] = [];
        const recommendations: string[] = [];
        const pinUsage: Record<string, string> = {}; // Changed to Object
        ...
        let match;
        const definedPins = new Map<string, string>(); // Name -> Number
